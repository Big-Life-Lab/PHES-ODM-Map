...
Grouping rows is an operation that would be performed before applying another operation. For example, we may want to group rows in a long table based on date, then feed these groups into the [Pivot Wider](#operation-pivot-wider) operation to pivot each group of rows into a single row representing all measurements for the given date. Grouping can be applied based on multiple columns. For example, we may want to group by date and compartment:
...
`r mdtable_csv_file("examples/odm/measure-singledate-odmlong.csv")`
...
`r mdtable_csv_file("examples/odm/measure-singledate-odmwide.csv")`
...
`r mdtable_csv_file("examples/odm/measure-singledate-odmlong.csv", rows = 1)`
